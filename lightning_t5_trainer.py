                    ...
                    for evidenceIdx, evidenceVal in enumerate(evidence):
                        if type(evidenceVal) == list:
                            ...
                            for idVal in evidenceVal:
                                ...
                                if args['use_topics']:
                                    matchedEvidenceText.append(
                                        " ".join(evidenceDf[evidenceDf['document'] == evidenceTitle]['text']))
                                else:
                                    matchedEvidenceText.append(
                                        evidenceDf[evidenceDf['combinedId'] == outId]['text'].values[0])

                    # TODO: Modify context and targets as needed
                    outArray.append({"context": context, "target": target, "goldEvidenceIds": matchedEvidenceIds,
                                     'evidenceText': matchedEvidenceText, 'examplePath': path})
...
class seqTrainer(pl.LightningModule):
    ...
    def collator(self, batch):
        tokenizedContext = self.tokenizer([x['context'] for x in batch], padding=True, truncation=True,
                                          max_length=self.args['max_tkn_length'], return_tensors='pt')
        input_ids = tokenizedContext['input_ids']
        attention_mask = tokenizedContext['attention_mask']
        labels = self.tokenizer([x['target'] for x in batch], padding=True, truncation=True,
                                max_length=self.args['max_tkn_length'], return_tensors='pt')['input_ids']
    ...
    def train_dataloader(self):
        return DataLoader(self.trainDs, batch_size=self.args['batch_size'], shuffle=True,
                          num_workers=self.args['num_workers'], collate_fn=self.collator)

    def val_dataloader(self):
        return DataLoader(self.devDs, batch_size=self.args['batch_size'], shuffle=False,
                          num_workers=self.args['num_workers'], collate_fn=self.collator)

    def test_dataloader(self):
        return DataLoader(self.testDs, batch_size=self.args['batch_size'], shuffle=False,
                          num_workers=self.args['num_workers'], collate_fn=self.collator)
...
def run_training(model, args):
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            ModelCheckpoint(monitor='val_loss', dirpath=args['model_directory'],
                            filename=args['logger_name'] + '-{epoch:02d}-{val_loss:.2f}', save_top_k=1, mode='min')
        ],
        logger=wandb_logger,
        deterministic=True
    )
...
def run_generation(model, args):
    ...
    for batchIdx in range(0, len(contexts), batchSize):
        ...
        tokenizedContext = model.tokenizer(batch, padding=True, truncation=True, max_length=args['max_tkn_length'],
                                           return_tensors='pt')
...
def run_interface(model, args):
    ...
    # Output warning
    print(
        "User Warning: This model has been trained to mirror the context that it has been given. If the user inputs a hateful, biased, or blatantly false context, the model will use that output to generate its response. You are adivised to input contexts only from sources you trust and to not take all statements generated from the model as absolute truth.")


use_gui = input("Would you Like to use GUI? (y/n): ")
# Using the gui interface if y, otherwise using the terminal interface
if use_gui.lower() == 'y':
    def echo(message, history, system_prompt):
        context = "Use the Article to answer the Question:" + message + " Article: " + system_prompt
        tokenizedContext = model.tokenizer(context, padding=True, truncation=True,
                                           max_length=args['max_tkn_length'],
                                           return_tensors='pt')
        input_ids = tokenizedContext['input_ids'].to(device)
        attention_mask = tokenizedContext['attention_mask'].to(device)
        outputs = model.model.generate(input_ids=input_ids, max_new_tokens=150)
        outputString = model.tokenizer.decode(outputs[0], skip_special_tokens=True)
        for i in range(min(len(outputString), int(512))):
            time.sleep(0.05)
            yield outputString[: i + 1]


    with gr.Blocks() as demo:
        system_prompt = gr.Textbox("Enter article context here.", label="Context")

        gr.ChatInterface(
            echo, additional_inputs=[system_prompt]
        )
    ...
    while True:
        ...
        tokenizedContext = model.tokenizer(context, padding=True, truncation=True,
                                           max_length=args['max_tkn_length'], return_tensors='pt')
        input_ids = tokenizedContext['input_ids'].to(device)
        attention_mask = tokenizedContext['attention_mask'].to(device)
        outputs = model.model.generate(input_ids=input_ids, max_new_tokens=150)
        outputString = model.tokenizer.decode(outputs[0], skip_special_tokens=True)
